    ...
    try {
        ...
        if (!env.GITHUB_TOKEN) {
            return {
                ...
                message: 'No GitHub authentication token found. Please configure GITHUB_TOKEN in wrangler.toml',
        ...
        if (!env.GITHUB_OWNER || !env.GITHUB_REPO) {
            return {
                ...
                details: `Missing: ${!env.GITHUB_OWNER ? 'GITHUB_OWNER' : ''} ${!env.GITHUB_REPO ? 'GITHUB_REPO' : ''}`.trim()
            };
        }
        ...
        if (theme.previewImage) {
            try {
                ...
                if (!previewResponse.ok) {
                    ...
                    console.error('Preview upload failed:', previewResponse.status, previewErrorText);
                    return {
                        success: false,
        ...
        if (response.ok) {
            ...
            if (response.status === 401) {
                ...
                userMessage = 'GitHub authentication failed. Please check your token permissions.';
            } else if (response.status === 403) {
                errorType = 'permissions';
                userMessage = 'Insufficient permissions to write to the GitHub repository.';
            } else if (response.status === 404) {
                errorType = 'repository_not_found';
                userMessage = 'GitHub repository not found. Please check repository name and permissions.';
            } else if (response.status === 422) {
                errorType = 'validation';
                userMessage = 'GitHub rejected the request. The file may already exist or the path is invalid.';
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            ...
            userMessage = 'Unable to connect to GitHub. Please check your internet connection.';
